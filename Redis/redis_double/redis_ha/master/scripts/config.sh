...
Config_Redis () {
\cp /opt/redis_ha/master/conf/redis_conf/redis.conf /etc/redis.conf
sed -i "s#192.168.3.177#${SLAVEIP}#g" /etc/redis.conf
}

Config_Keep () {
\cp /opt/redis_ha/master/conf/keep_conf/keepalived.conf.master /etc/keepalived/keepalived.conf
